...
class MongoTrackerStore(TrackerStore, SerializedTrackerAsText):
    ...
    def messages(self) -> Collection:
        """Returns the current conversation."""
        return self.db[self.chatHistory]
    ...
    @staticmethod
    def _optimize_tracker_store_for_chat_history(tracker: DialogueStateTracker, messages: Iterator) -> Dict:
        # get current tracker state and remove `events` key from state
        # since events are pushed separately in the `update_one()` operation
        state = tracker.current_state(EventVerbosity.ALL)
        state.pop("active_loop", None)
        state.pop("followup_action", None)
        state.pop("latest_action", None)
        state.pop("latest_action_name", None)
        state.pop("latest_input_channel", None)
        state.pop("latest_message", None)
        state.pop("paused", None)
        state.pop("events", None)
        state['bot_messages'] = [e.as_dict() for e in messages]
    ...
    def saveChatHistory(self, tracker: DialogueStateTracker, messages: Iterator) -> None:
        """Saves the current message history."""

        self.messages.update_one(
            {"sender_id": tracker.sender_id},
            {
                "$set": self._optimize_tracker_store_for_chat_history(tracker, messages)
            },
            upsert=True,
        )
    ...
    def _additional_events(self, tracker: DialogueStateTracker) -> Iterator:
        ...
        storedMessages = self.messages.find_one(
            {"sender_id": tracker.sender_id}) or {}
        all_events = self._events_from_serialized_tracker(stored)
        all_bot_messages = self._bot_messages_from_serialized_tracker(
            storedMessages)
        ...
        # eventslist = list(itertools.islice(tracker.events, 0, 1))
        # logger.debug(eventslist)

        filteredList = deque()

        for i in tracker.events:
            if (i.type_name == 'bot' or i.type_name == 'user'):
                filteredList.append(i)
                logger.debug(i)

        self.saveChatHistory(tracker, itertools.islice(
            filteredList, 0, len(filteredList)))
    ...
    @staticmethod
    def _bot_messages_from_serialized_tracker(serialised: Dict) -> List[Dict]:
        return serialised.get("events", [])
